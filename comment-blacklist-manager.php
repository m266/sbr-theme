...
class SBR_CBL_Manager_Core
{
	...
	public function load_settings() {
		...
		add_settings_field( 'blacklist-source', __( 'Blacklist Quelle', 'comment-blacklist-manager' ), array( $this, 'source_field' ), 'discussion', 'default' );

		// load the custom list field
		add_settings_field( 'blacklist-local', __( 'Lokale Blacklist', 'comment-blacklist-manager' ), array( $this, 'local_field' ), 'discussion', 'default' );

		// load the exclusion field
		add_settings_field( 'blacklist-exclude', __( 'Ausgenommene Begriffe', 'comment-blacklist-manager' ), array( $this, 'exclude_field' ), 'discussion', 'default' );
	...
	public function source_field() {
		...
		echo '<p>';
			echo '<label>' . __( 'Die Daten von den unten angef&uuml;hrten Quellen werden automatisch in die Kommentar-Blacklist geladen.', 'comment-blacklist-manager' ) . '</label>';
		echo '</p>';
		...
		echo '<a class="button button-secondary" href=" ' . admin_url( 'options-discussion.php' ) . '?cblm-update=manual">' . __( 'Aktualisierung von Hand starten', 'comment-blacklist-manager' ) . '</a>';
	...
	public function local_field() {
		...
		echo '<p>';
			echo '<label for="blacklist_local">' . __( 'Jeder der unten aufgef&uuml;hrten Begriffe wird zu den Daten aus den Blacklist-Quellen hinzugef&uuml;gt. Ein Wort oder eine IP pro Zeile. Die Begriffe werden innerhalb von Worten erkannt: "press" findet auch "WordPress".', 'comment-blacklist-manager' ) . '</label>';
		echo '</p>';
	...
	public function exclude_field() {
		...
		echo '<p>';
			echo '<label for="blacklist_exclude">' . __( 'Jeder der unten aufgef&uuml;rten Begriffe wird aus den Blacklist-Aktualisierungen ausgenommen. Ein Wort oder eine IP pro Zeile. Die Begriffe werden innerhalb von Worten erkannt: "press" findet auch "WordPress".', 'comment-blacklist-manager' ) . '</label>';
		echo '</p>';
...
$CBL_Manager_Core = SBR_CBL_Manager_Core::getInstance();
